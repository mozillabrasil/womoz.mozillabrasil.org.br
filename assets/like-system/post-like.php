...
function like_scripts() {
	...
	wp_enqueue_script( 'jm_like_post', get_template_directory_uri().'/assets/like-system/js/post-like.min.js', array('jquery'), '1.0', 1 );
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'Você Curtiu:' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . _e( 'Você não curtiu nada ainda.' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . __( 'Você Curtiu:' ) . "</h3>\n";
			$like_list .= "<ul>\n";
